...
fun readConfigFromJson(): Map<String, String> {
    ...
    val defaults = mapOf(
        ...
        "admobAppId" to "ca-app-pub-5202253201958912~5352519435"
    )
...
val admobAppIdFromConfig = appConfig["admobAppId"] ?: "ca-app-pub-5202253201958912~5352519435"
...
android {
    ...
    signingConfigs {
        create("release") {
            // Only wire up release signing when key.properties is present
            // (e.g. on the build machine / Codemagic). Fresh clones without it
            // can still build & run debug without this block crashing at
            // evaluation time on the `as String` casts.
            if (keystorePropertiesFile.exists()) {
